<?php
	...
	$selCountSent = "select failed_messages.*, students.*, message_types.*, date_format(failed_messages.created_at, '%b %e, %Y [ %H:%i:%s ]') as date_sent from failed_messages inner join students on students.id = failed_messages.student_id inner join message_types on message_types.id = failed_messages.message_type_id where date_format(failed_messages.created_at, '%Y-%m-%d') between '" . $startdate . "' and '" . $enddate . "'  order by failed_messages.created_at";
				...
				<div class="per-page">
					<?php if ($x == 1): ?>
						<div class="text-center">
							<h2 class="mb-5"><?php echo 'Failed Message Report' ?></h2>
						...
						<table class="table">
							...
							<tbody style="color: #333">
								<?php
									...
									$selCountSent = "select failed_messages.*, students.*, message_types.*, date_format(failed_messages.created_at, '%b %e, %Y [ %H:%i:%s ]') as date_sent from failed_messages inner join students on students.id = failed_messages.student_id inner join message_types on message_types.id = failed_messages.message_type_id where date_format(failed_messages.created_at, '%Y-%m-%d') between '" . $startdate . "' and '" . $enddate . "'  order by failed_messages.created_at limit " . $limit . "," . $rowPerPage;
